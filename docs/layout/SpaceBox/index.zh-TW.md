...
與 [Ant Design](https://ant.design) 的 [Space](https://ant.design/components/space) 組件類似，`SpaceBox` 用於提供內間距包裹子組件。
但不同的是，`Space` 用於間隔兩個或兩個以上的組件，`SpaceBox` 可用於間隔任意數量的子組件，哪怕只有壹個子組件。
